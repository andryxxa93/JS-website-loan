...
window.addEventListener('DOMContentLoaded', function () {
  ...
  difference.init();
  var forms = new _modules_forms__WEBPACK_IMPORTED_MODULE_4__["default"]('.form');
  forms.init();
...
function () {
  ...
  _createClass(Forms, [{
    ...
    value: function checkMailInputs() {
      var textInputs = document.querySelectorAll('[name="email"]');
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        // Создаём функцию, которая принимает позицию и элемент на котором нужно установить крусор
        elem.focus(); // Устанавливаем курсор на элемент

        if (elem.setSelectionRange) {
          // Если браузер поддерживает функию, то используем её для установки курсора в нужную позицию
          elem.setSelectionRange(pos, pos);
        } else if (elem.createTextRange) {
          // Если не поддерживает создаём полифил
      ...
      function createMask(event) {
        // Функция принимающая событие
        var matrix = '+1 (___) ___-____',
            // матрица номера телефона
        i = 0,
            def = matrix.replace(/\D/g, ''),
            // вынимаем из матрицы все не числа
        ...
        this.value = matrix.replace(/./g, function (a) {
          // перебираем все символы в текущем инпуте
